...
Every editable value (qubit/pair inspector rows; focused bulk-grid cells) carries a 🕘 **value-history popover** — `GET /field/history?path=` → `HistoryManager.field_history`: change-point timeline per dot-path (tracked props via the SQLite index; any other leaf via a capped direct scan of snapshot state/wiring copies; pointers resolved per-snapshot), each row naming the experiment that introduced the value, with **Use** (fills the edit input, commit stays user-explicit) and **Data** (loads the run's detail into `#inspector-pane`). See the docs/20 amendment (2026-07-30).

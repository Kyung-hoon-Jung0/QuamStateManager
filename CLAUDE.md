...
Tests use `tmp_path` synthetic fixtures. Some tests additionally probe real data from `<data-root>\...` and auto-skip when that path is absent. No test config files — pytest uses default discovery. An autouse fixture in `tests/conftest.py` points `QUALIBRATE_CONFIG_FILE` at a nonexistent tmp path so the suite never reads the developer's real `~/.qualibrate` (docs/63). **4008 pass, 199 skip, 0 fail** on the `qm_mng` env (2026-07-27; the skipped tests are real-data tests gated by the ExampleChip folder's presence; the waveform-golden live-regeneration test additionally needs the Windows conda `LabC` env at `<qm-env>/python`; `test_main.py::TestWaitForServer` needs working WSL→Windows loopback and fails environmentally on machines where that's broken).

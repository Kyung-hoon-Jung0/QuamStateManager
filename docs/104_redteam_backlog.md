...
|---|---|---|---|
| 1 | **CRITICAL — a live-file write with no confirm**: the tray's ⚡ *Apply to live now* (`change_count>0, not working_dirty` — exactly a first-time user's state after the chip-name banner stages edit #1) called `doStateSync('apply')` bare, while its `working_dirty` sibling always confirmed (new user #1/#2) | one confirm naming the act + reversibility; the review modal's Pull & apply stays unconfirmed there (the diff is on screen) | this branch |

...
DOC_ENTRIES: list[dict] = [
    ...
    # ── analog output ports, all controllers ───────────────────────────
    {
        ...
        "allowed": None, "default": 0.0, "unit": "V",
    ...
    },
    {
        ...
        "summary": "DC offset correction on the analog input port, in V (the controller example configures inputs with `offset`).",
        "allowed": None, "default": 0.0, "unit": "V",
        "docs": f"{_CONFIG}#controllers",
    ...
    },
    {
        ...
        "summary": "A fixed delay added to everything played from this analog output port, in ns (OPX+: QOP ≥ 2). Used to align lines of different cable length.",
        "allowed": None, "default": 0, "unit": "ns",
        "docs": f"{_CONFIG}#controllers",
    ...
    },
    {
        ...
        "summary": "Measurement timing: the delay, in ns, between playing the readout pulse and the returned signal reaching the ADC (cable + device round trip); the acquisition window starts after it.",
    ...
    },
    {
        "key": "smearing",
        "applies": ["InOutIQChannel", "InOutMWChannel", "InOutSingleChannel", "ReadoutResonator"],
        "summary": "Measurement timing: extra samples, in ns, acquired on both sides of the readout window to catch a signal smeared by the line's response.",
        "allowed": None, "default": None, "unit": "ns",
        "docs": f"{_DEMOD}#timing-of-the-measurement-operation",
        "quote": "The `time_of_flight` and `smearing` keys are parameters related to the timing of the signal",
    },
    ...
    },
    {
        "key": "thread",
        "applies": ["Channel"],
        "summary": "The OPX+ name of the same idea as `core`: elements assigned the same thread share a pulse-processor thread and must never be played together.",
        "allowed": None, "default": None, "unit": None,
        "docs": f"{_FEATURES}#cores-and-oscillators",
        "quote": "By default, the different cores & oscillators are not shared between different elements.",
    },
    # ── digital ───────────────────────────────────────────────────────
    {
        "key": "delay",
        "applies": ["DigitalOutputChannel"],
        "summary": "Delay of the digital marker relative to the analog pulse of its element, in ns (the element's `digitalInputs` entry).",
        "allowed": None, "default": None, "unit": "ns",
        "docs": f"{_CONFIG}#elements",
        "quote": "'digitalInputs': {'switch_in': ('con1', 1)}",
    },
    {
        "key": "buffer",
        "applies": ["DigitalOutputChannel"],
        "summary": "Extends the digital marker on both sides of the analog pulse, in ns (the element's `digitalInputs` entry).",
        "allowed": None, "default": None, "unit": "ns",
        "docs": f"{_CONFIG}#elements",
        "quote": "'digitalInputs': {'switch_in': ('con1', 1)}",
    },

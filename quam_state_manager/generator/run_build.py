...
def _set_channel_band(channel, band):
    """Band-only override (no LO edit): write the band onto the channel's
    output + paired input ports. r16 ⓪-3."""
    if band not in (1, 2, 3):
        return
    for port in (getattr(channel, "opx_output", None),
                 getattr(channel, "opx_input", None)):
        if port is not None and hasattr(port, "band"):
            try:
                port.band = band
            except ValueError:
                pass                      # reference-backed attr — propagates
...
def run_build(spec: dict, out_dir: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="quam_build_") as _bd:
        ...
        if ((spec.get("populate") or {}).get("options") or {}).get("pin_cores"):
            _pin_cores(machine)
        # docs/141 4ak: say it when a declared pair produced nothing. Silence
    ...
    return {
        ...
        # docs/141 4ak: a pair the spec DECLARED that no pair exists for. A pair

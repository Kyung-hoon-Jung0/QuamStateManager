...
(async function main() {
  ...
  // ── r2-14: the Report menu closes like every other popup ────────────────
  {
    ...
    ok(T.win.ChipStatus.reportHref(a, 'md') === true
       && /\/topology\/report\?format=md$/.test(a.href),
       'r2-14 the format link is still rewritten (QA F-14: format only, the lab spec scores it)');
    await sleep(10);

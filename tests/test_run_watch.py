...
class TestWaitRoute:
    ...
    def test_the_handshake_answers_at_once_and_never_as_a_change(self, wake_client):
        ...
        assert d == {"tick": 0, "changed": False, "agent_changed": False, "roots": d["roots"], "agent_seq": d["agent_seq"]} \
        ...
        assert d["changed"] is True and d["tick"] == 1
        d = c.get("/datasets/wait?since=-1&timeout=25").get_json()
        assert d == {"tick": 1, "changed": False, "agent_changed": False, "roots": d["roots"], "agent_seq": d["agent_seq"]}

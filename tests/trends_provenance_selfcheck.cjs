...
const SNAPS = {
  ...
  '20260901_010100': { run: 34, node: '03_resonator_spectroscopy_single',
                       short: 'Res spec', why: null, uid: 'a1b2c3d4:34' },
  '20260901_010200': { run: 99, node: '06_ramsey', short: 'Ramsey',
                       why: null, uid: null },
};
...
world.push((function () {
  ...
  return drawn(win).then(function () {
    ...
    ok(tr.customdata[1][1] === '#34 · Res spec',
       '3d a run point names the run and a short node name');
    ok(/click to open the dataset/.test(tr.customdata[1][2]),
       '3e ...with the click hint, because it has a uid');
    ok(tr.customdata[2][1] === '#99 · Ramsey' && tr.customdata[2][2] === '',
